...
class MainView: UIViewController {
  ...
  @objc func didTapButton() {
    guard let newList = viewModel?.createNewList() else { return }
    viewModel?.coordinator?.navigateToShoppingView(with: newList)
  }
  ...
  private func setupConstraints() {
    ...
    if let savedLists = viewModel?.savedLists,
       savedLists.isEmpty {
      ...
      print(viewModel?.savedLists[0].title as Any)
    }
  }
}
